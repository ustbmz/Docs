...
- 节点类型：
  - React DOM节点(ReactDomComponents)：创建该节点的React元素类型是一个字符串
  - React 组件节点(ReactCompsite)：创建该节点的React元素类型是一个函数或是一个类
  - React 文本节点(React Text)：由字符串、数字创建的
  - React 空节点(React Empty)：由null、undefined、false、true
  - React 数组节点(React Array)：该节点由一个数组创建
- 真实DOM：通过document.createElement创建的dom元素

![](https://s2.loli.net/2024/08/04/9H7xSTUPjARGEd3.png)
...
![](https://s2.loli.net/2024/08/04/ClG3Wc4sA5ZDLNR.png)
...
![](https://s2.loli.net/2024/08/04/vLy1omfd7XA8hPY.png)
...
![](https://s2.loli.net/2024/08/04/TwMsgijf6Uk8rOt.png)
...
export default class classRender extends Component {
  ...
  componentDidMount() {
    console.log('🐤 ≂ componentDidMount: classRender')
  }
  componentWillUnmount() {
    console.log('🐤 ≂ componentWillUnmount: classRender')
  }
  render() {
    console.log('🐤 ≂ render: classRender')
    // 尽量不要改变节点类型和节点结构,就可以提高渲染效率
    const h1 = this.state.show ? <div>标题</div> : null
    return (
      <div>
        {this.state.show ? (
          <div>
